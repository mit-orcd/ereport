...
```bash
make fuse-headers && make ecrawl_mount   # only if fuse-devel is not installed
./ecrawl_mount crawl-out ~/mnt           # read-only; unmount with: fusermount -u ~/mnt
find ~/mnt -mtime +365 -size +1G         # any POSIX tool works
...
- [Crawl shard binary format](docs/binary-format.md) — `ERCBIN08` header, columnar zstd-compressed row groups with per-column zone maps, catalog tail with DFS ordering and subtree rollups, and `.ckpt` sidecars.
