...
static void usage(void) {
    fprintf(stderr,
            ...
            "\n"
            "Mount a crawl as a read-only, metadata-only filesystem.\n"
            "Unmount: fusermount -u <mountpoint>\n"
            "\n"
            "Options:\n"
            "  -o path=DIR          crawl directory (or use the positional form)\n"
            "  -o subtree=PATH      mount only this subtree (also shrinks the index)\n"
            ...
            "  -d                   FUSE debug (implies -f)\n"
            "  -s                   single-threaded FUSE event loop\n"
            "  -v, --verbose        index build progress on stderr\n"
            "  -h, --help           this message\n"
            "\n"
            "read() returns zeros; permissions 0555/0444; readlink() is EIO.\n"
            "Sizes, timestamps, uid, nlink, and inodes are exact.\n",
