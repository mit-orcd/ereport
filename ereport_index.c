...
static void die_usage(const char *argv0) {
    fprintf(stderr,
            "Usage: %s --make          [options] [user] [bin_dir ...]\n"
            "       %s --search        [options] <term>\n"
            "       %s --resume-merge --index-dir DIR\n"
            "\n"
            "  --make            build the trigram index (and dirs.idx / rowgroups.idx)\n"
            "  --search          case-insensitive substring search (min 3 chars)\n"
            "  --resume-merge    finish a crashed --make merge\n"
            "\n"
            "Options:\n"
            ...
            "  --limit M               --search: return at most M hits\n"
            "  --verbose               --make/--resume-merge: detailed stderr; --search: timing\n"
            "\n"
            "  user     login or uid; omit for all-users (./all_users/index/)\n"
            "  bin_dir  crawl directories; default: cwd.  Same merge rules as ereport.\n"
            "\n"
            "Environment:\n"
            "  EREPORT_INDEX_THREADS           --search filter / --make default (default %d)\n"
            "  EREPORT_INDEX_TRIGRAM_THREADS   --make tmp-trigram writers (default: same)\n"
            "\n"
            "--resume-merge needs --index-dir.  Large --make: ulimit -n 65535 and ulimit -f unlimited.\n"
            "Details: docs/tools.md.\n",
            argv0, argv0, argv0, DEFAULT_THREADS);
    exit(2);
}
