...
- That scan also takes its membership test from the sidecar: each record's `parent_dir_id` is mapped to its DFS position by a single catalog-row read through the sidecar's chunk table (cached per worker, and records arrive sorted by parent, so a chunk is decoded at most once per task), then tested against the subtree's DFS ranges. No shard catalog is materialized — `catalogs_loaded=0` in the stats block — where the old route parsed every catalog row in every scanned shard. `--list` still loads catalogs, because printing paths needs the name columns anyway.
...
Work is split on row-group boundaries so `--writers` can run in parallel on a single shard. Directories are created once from the catalog before writers start. Because a row group's kept bytes vary wildly (especially under `--only`, where most row groups contribute nothing and a few hold gigabytes), record scanning is decoupled from object creation: scanners resolve names and paths and enqueue one work item per kept object, and every writer thread drains the shared queue (a scanner that finds the queue full executes one item itself before retrying, so backpressure is self-balancing). Hardlink claims still happen at scan time; a losing claim unblocks only after the winner's write completed, so queued `link(2)` items always find their target. Each writer also pwrites from its own private copy of the content block: with one shared buffer the kernel pins/unpins the same pages from every thread (GUP folio-refcount contention), which throttles Direct I/O submission at high writer counts.
...
While dumping, a live `files/dirs/other/volume/elapsed` line rewrites itself on stderr about once a second (default on when stderr is a TTY; `--progress` forces it on for logs, `--no-progress` off). A reporter thread only reads the counters the workers already maintain, so progress costs the dump hot path nothing. `files` and `dirs` count completed regular files and directories; `other` counts the remaining object types (symlinks + fifos + device nodes + hardlinks), which are often rare — it is not a grand total. `volume` counts bytes as they are written (one relaxed atomic per write syscall), so it moves smoothly even while the writers are inside a few giant files; `files`/`other` count completed objects.
...
- `--sunburst-buckets` (off by default) adds per-node age × size breakdowns: after the tree is built, a second pass over the bins re-reads every record with the same filters and credits a 6×6 matrix (the report's age rows × size columns) on the node the record rolled up to. The HTML page then shows two chip rows — six age ranges, six size ranges, all on initially — and toggling a chip re-slices every wedge to the selected cells; a "Color" selector can paint wedges by their dominant age or size bucket instead of by directory. Per node the matrix sums equal `bytes`/`files` exactly (hardlink dedup is replayed from winner record-ids stored during the main scan, so the hardlink pair counts once in exactly one cell). Cost when enabled: one extra read of the bins (narrow column projection, no path strings unless `--subtree`/`--bucket-details` already need them) plus roughly one pointer per directory and 8 bytes per hardlink inode of extra memory.
- A collapsible **Legend** below the chart explains the color schemes: the directory palette (per top-level directory hue, lighter at depth; grey for `(self)`/`(other)` wedges) always, and the age/size bucket scales when `--sunburst-buckets` is on.
- On all-users runs the sunburst page also gets a **User picker** (top bar): one sunburst tree is built per seen uid and written to `all_users/users/<name>.{html,json}` (sorted by bytes in the menu, "all users" first), and picking a user opens that user's page — same chart, same filters, bucket matrices included when `--sunburst-buckets` is on. Each uid's records live in exactly one shard, so a per-user tree costs one single-shard build after the aggregate build; registration stops past a 4 GiB accumulator ceiling (a warning names it, and those users just get no page). `--no-sunburst-users` turns the whole thing off. Single-user runs never get the picker.
- `sunburst.json` schema: one root node, recursively `{ "name", "path", "bytes", "files", "children"? }`. Internal nodes carry *self* values (records directly in that directory); leaves carry *subtree totals* (a depth-folded leaf includes everything below it). So a node's total is its own `bytes`/`files` plus the sum over its children, and the plain sum of every `bytes` value in the document is the grand total. Names are JSON-escaped (`<`, `>`, `&` included, so the same JSON is safe to embed in HTML); numbers are unsigned 64-bit — JavaScript treats them exactly only below 2^53. With `--sunburst-buckets` each node also carries `"bucket_bytes"` and `"bucket_files"`: 36 values each, `[age_bucket][size_bucket]` row-major over the report's bucket axes, mirroring the node semantics (self values for internal nodes, subtree totals for leaves) and summing exactly to the node's `bytes`/`files`.
