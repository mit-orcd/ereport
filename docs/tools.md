...
```bash
./ereport [--bucket-details N] [--subtree PATH] [--index-dir DIR] [--no-sunburst] [--sunburst-depth N] [--sunburst-buckets] <username|uid> [<atime|mtime|ctime|effective>] [bin_dir ...]
./ereport [--bucket-details N] [--subtree PATH] [--index-dir DIR] [--no-sunburst] [--sunburst-depth N] [--sunburst-buckets] [<atime|mtime|ctime|effective>] [bin_dir ...]   # all users → ./all_users/
```
