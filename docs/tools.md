...
While dumping, a live `files/dirs/objects/volume/elapsed` line rewrites itself on stderr about once a second (default on when stderr is a TTY; `--progress` forces it on for logs, `--no-progress` off). A reporter thread only reads the counters the workers already maintain, so progress costs the dump hot path nothing.
